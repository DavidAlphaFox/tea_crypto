...
static ERL_NIF_TERM tea_ntohl_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]){
  ErlNifBinary value,bin;
  if(!enif_inspect_binary(env, argv[0], &value) || 4 != value.size){
     return enif_make_badarg(env);
  }

  if(!enif_alloc_binary(4,&bin)){
    enif_release_binary(&value);
    return enif_make_badarg(env);
  }
  uint32_t* msg_buff = (uint32_t*)enif_alloc(sizeof(uint32_t));
  uint32_t ntohl_int = 0;
  memcpy(msg_buff,(value.data),4);
  ntohl_int =  ntohl(*msg_buff),
  enif_release_binary(&value);

  memcpy((bin.data),&ntohl_int,4);
  enif_free(msg_buff);
  return enif_make_binary(env, &bin);
}

static ERL_NIF_TERM tea_htonl_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]){
  ErlNifBinary value,bin;
  if(!enif_inspect_binary(env, argv[0], &value) || 4 != value.size){
     return enif_make_badarg(env);
  }
...
static ErlNifFunc nif_funcs[] = {
  ...
  {"tea_htonl",1,tea_htonl_nif},
  {"tea_ntohl",1,tea_ntohl_nif}
};
